...
function makeRibbons(val, val2, h)
{
  ...
  background('#202020');
    ...
    {
      var sinoffset = sin((frequency*i)*-c);
      var sinX = c*(width/amount);
      var sinY = startY + (sinoffset*offset);
      sumWave[c] = (sumWave[c] + (sinY*(1/(h))))*2;
    ...
    for (var d=1; d < amount; d++)
    {
      ...
      //sumWave[c] += sinY;
      bezierVertex(sinX2+40,startY+(50)+(sumWave[d]/(h/((h*2)-1))),sinX2+40,startY+(50)+(sumWave[d]/(h/((h*2)-1))),sinX2+40,startY+(50)+(sumWave[d]/(h/((h*2)-1))));
    }
    endShape();
    ...
    if (h > 1)
    {
      text("Fundemental frequency and harmonics", 200, 100);
    }
    else
    {
      text("Fundemental frequency", 250, 100);
    }

    text("Sum of funtemental frequency and harmonics", 170, startY+160);
